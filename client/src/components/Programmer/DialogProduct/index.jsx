...
export default function DialogProduct({
  ...
  const handleCharge = () => setChange(!change);
  const onSubmit = (values) => {
    if (change) {
      const productData = {
        id: product.idProducto,
        min_kg_carga: values.charge,
      };
      dispatch(updateProductsRequest(productData));
    } else {
      const pedido = () => {
        if (values.divider) {
          const load = parseInt(values.load);
          const loadExtra = parseInt(values.load_extra);
          return (load + loadExtra) * product?.min_kg_carga;
        } else {
          return parseInt(values.load) * product?.min_kg_carga;
        }
      };
      ...
      //   }
      // } else {
      //   if (values.baler !== values.baler_extra) {
      //     setRealPlan([
      //       ...realPlan,
      //       {
      //         idProducto: product?.idProducto,
            ...
            <Fragment>
              ...
              <div className="flex flex-col justify-center items-center">
                <TextField
                  ...
                  type="number"
                  size="small"
                  defaultValue={product?.min_kg_carga || 0}
